...
{
    [AbletLayer]
    public class FinishManualExportLayer : IAbletLayer
    {
        string IAbletDefinitionBase.Id => "ablet.layer.finish-manual-export";
        string IAbletDefinitionBase.DisplayName => "Finish Manual Export";
        int IAbletDefinitionBase.Priority => 0;
        void IAbletLayer.Configure(IDependencyConfigurator config)
        {
            config.AddDependency<AfterLayer<PhaseContainer>>();
        }

        IAbletProcessor IAbletLayer.Processor(BuildArgument argument) => argument.WillPersistGeneratedAssets ? new FinishManualExportProcessor() : null;
    }

    public class FinishManualExportProcessor : IAbletProcessor
    {
        void IAbletProcessor.Process(BuildContext buildContext)
        {
            try
            {
                ...
                if (!AssetDatabase.IsValidFolder("Assets/AbletOutput"))
                {
                    AssetDatabase.CreateFolder("Assets", "AbletOutput");
                }
                var prefabPath = AssetDatabase.GenerateUniqueAssetPath($"Assets/AbletOutput/{buildContext.CurrentRootObject.name}.prefab");
                var assetName = Path.GetFileNameWithoutExtension(prefabPath);
                if (!AssetDatabase.IsValidFolder($"Assets/AbletOutput/{assetName}"))
                {
                    AssetDatabase.CreateFolder("Assets/AbletOutput", assetName);
                }
                var container = ScriptableObject.CreateInstance<GeneratedAssets>();
                AssetDatabase.CreateAsset(container, $"Assets/AbletOutput/{assetName}/{assetName}.asset");
                foreach (var obj in IterateHierarchyAssetReferences(buildContext.CurrentRootObject))
                {
                    AssetDatabase.AddObjectToAsset(obj, container);
                }
                PrefabUtility.SaveAsPrefabAssetAndConnect(buildContext.CurrentRootObject, prefabPath, InteractionMode.AutomatedAction);
            }
            finally
            {
        ...
        static IEnumerable<Object> IterateHierarchyAssetReferences(GameObject rootGameObject)
        {
            return rootGameObject.ReferencedAssets(traverseSaved: false, includeScene: true)
                .Where(asset => asset is not MonoScript and not Component and not GameObject);
        }
    }
